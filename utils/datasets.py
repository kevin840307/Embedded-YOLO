...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False, local_rank=-1, world_size=1, multiple_augmentation=False):
    ...
    with torch_distributed_zero_first(local_rank):
        dataset = LoadImagesAndLabels(path, imgsz, batch_size,
                                    ...
                                    stride=int(stride),
                                    pad=pad,
                                    multiple_augmentation=multiple_augmentation)
...
hue_transform = {
    'train': transforms.Compose([
        transforms.RandomApply([transforms.ColorJitter(hue=0.3)], p=0.5),
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, stride=32, pad=0.0, multiple_augmentation=False):
        try:
            f = []  # image files
        ...
        self.epoch = 0
        self.multiple_augmentation = multiple_augmentation
        self.hue_transform = hue_transform['train']
        self.multiple_transform = multiple_transform['train']
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            if self.multiple_augmentation:
                img = self.multiple_transform(img)
            else:
                img = self.hue_transform(img)
            img = np.array(img)
...
def load_mosaic(self, index):
    ...
    if self.epoch == None:
        ...
        arr = np.arange(0, len(self.labels))
        np.random.RandomState(self.epoch + 1234).shuffle(arr)
        indices = [index] + [arr[(index + 1) % len(self.labels)],
                             arr[(index + 2) % len(self.labels)],
                             arr[(index + 3) % len(self.labels)]]
    top_margin = 99999
    left_margin = 99999
    ...
    img4, labels4 = random_affine(img4, labels4,
                                  ...
                                  border=self.mosaic_border, margin=[top_margin, left_margin])  # border to remove
...
def random_affine(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, border=(0, 0), margin=[99999, 99999]):
    ...
    T = np.eye(3)
    T[0, 2] = random.uniform(-translate, translate) * img.shape[1] + border[1]  #(border[1] - left_margin) // 2 # x translation (pixels)
    T[1, 2] = random.uniform(-translate, translate) * img.shape[0] + (border[0] - top_margin) // 2  # y translation (pixels)
